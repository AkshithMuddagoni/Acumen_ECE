...
const MediaItem = ({ item, className, onClick, isHovered = false, isExpanded = false }: { item: MediaItemType, className?: string, onClick?: () => void, isHovered?: boolean, isExpanded?: boolean }) => {
    const videoRef = useRef<HTMLVideoElement>(null); // Reference for video element
    const [isBuffering, setIsBuffering] = useState(false);  // To track if video is buffering

    // Only play video on hover (grid) or in expanded modal
    useEffect(() => {
        let mounted = true;
        ...
        // Play only if hovered or expanded
    ...
    if (item.type === 'video') {
        return (
            <div className={`${className} relative overflow-hidden bg-gray-900`}>
                ...
                {/* Play icon indicator */}
                <div className="absolute inset-0 flex items-center justify-center opacity-40 hover:opacity-100 transition-opacity duration-300">
                    ...
                    <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-gray-50/50">
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                onClick={onClose}
                            >
                                <MediaItem item={selectedItem} className="w-full h-full object-contain bg-gray-900/20" onClick={onClose} isExpanded={true} />
                ...
                >
                    <div className="flex items-center -space-x-2 px-3 py-2">
                        {mediaItems.map((item, index) => (
                            ...
                            >
                                <MediaItem item={item} className="w-full h-full" onClick={() => setSelectedItem(item)} isExpanded={true} />
