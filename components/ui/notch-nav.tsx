...
export function NotchNav({
  ...
  return (
    <nav aria-label={ariaLabel} className={["w-fit mx-auto", className].filter(Boolean).join(" ")}>
      <div ref={containerRef} className="relative rounded-lg border border-[#00ffaa]/30 bg-gradient-to-b from-[#1a1a3d]/40 to-[#0f0f25]/40 backdrop-blur-sm text-white">
        {/* Items */}
        <ul
          role="menubar"
            ...
            return (
              <li key={item.value} role="none">
                <button
                  ...
                  className={[
                    "relative rounded-md px-4 py-2 text-sm font-semibold outline-none transition-colors",
                    "focus-visible:ring-2 focus-visible:ring-[#00ffaa]",
                    isActive 
                      ? "text-white" 
                      : "text-gray-300 hover:text-white hover:bg-[#00ffaa]/10",
                  ].join(" ")}
                >
